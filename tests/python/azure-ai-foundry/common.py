...
def run_memory_operations(client):
    ...
    Spans are created in the prototype test to demonstrate what an instrumentation library
    ...
    with tracer.start_as_current_span("create_memory_store", kind=SpanKind.CLIENT) as span:
        ...
        span.set_attribute("gen_ai.memory.store.id", store_name)
        try:
            ...
            print(f"    -> created store: {result['name']}")
        except Exception as exc:
            span.set_status(StatusCode.ERROR, str(exc))
            span.set_attribute("error.type", type(exc).__qualname__)
            raise
    ...
    with tracer.start_as_current_span("update_memory", kind=SpanKind.CLIENT) as span:
        ...
        try:
            ...
            operation_result = update_result.get("result", update_result)
            operations = operation_result.get("memory_operations", [])
            if operations:
