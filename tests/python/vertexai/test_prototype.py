...
def run_chat():
    ...
    print("  [chat] basic chat completion via Vertex AI (prototype)")
...
def run_chat_streaming():
    ...
    print("  [chat_streaming] streaming chat completion via Vertex AI (prototype)")
    request_model = "gemini-2.0-flash"
    with _prototype_tracer.start_as_current_span("chat gemini-2.0-flash") as span:
        span.set_attribute("gen_ai.operation.name", "chat")
        span.set_attribute("gen_ai.provider.name", "vertex_ai")
...
def main():
    print("=== Prototype: Vertex AI Conformance Test ===")
