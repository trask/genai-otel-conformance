...
def run_chat_reference(client):
    """Scenario: basic message with reference instrumentation."""
    print("  [chat] basic message (reference)")
...
def run_chat_streaming_reference(client):
    """Scenario: streaming message with reference instrumentation."""
    print("  [chat_streaming] streaming message (reference)")
    request_model = "claude-sonnet-4-20250514"
    with _reference_tracer.start_as_current_span("chat claude-sonnet-4-20250514") as span:
        span.set_attribute("gen_ai.operation.name", "chat")
        span.set_attribute("gen_ai.provider.name", "anthropic")
...
def main():
    print("=== Reference: Anthropic Conformance Test ===")
