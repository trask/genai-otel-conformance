...
Exercises: agent_run with tool calling.
"""
...
class OTelBridgeExporter(TracingExporter):
    ...
    @staticmethod
    def _to_ns(dt):
        """datetime or ISO string  ->  nanoseconds since epoch (or None)."""
        if dt is None:
            return None
    ...
    def _build_parent_ctx(self, agent_span):
        ...
        raw_trace = agent_span.trace_id
        raw_parent = agent_span.parent_id
        if isinstance(raw_trace, str) and not raw_trace.startswith("0"):
            raw_trace = raw_trace.rsplit("_", 1)[-1]
        if isinstance(raw_parent, str) and not raw_parent.startswith("0"):
            raw_parent = raw_parent.rsplit("_", 1)[-1]
        parent_span_ctx = SpanContext(
            trace_id=int(raw_trace, 16),
            span_id=int(raw_parent, 16),
            is_remote=False,
    ...
    def _collect_attributes(span_data):
        ...
        if isinstance(span_data, AgentSpanData):
            ...
            if span_data.output_type:
                attrs["gen_ai.agent.output_type"] = str(span_data.output_type)
            ...
            if span_data.input:
                attrs["gen_ai.tool.input"] = str(span_data.input)
            if span_data.output:
                attrs["gen_ai.tool.output"] = str(span_data.output)
            ...
            if span_data.data:
                attrs["gen_ai.custom.data"] = str(span_data.data)
        return attrs
    ...
    def _export_span(self, agent_span):
        ...
        otel_span = self._tracer.start_span(
            name, context=parent_ctx, attributes=attributes, start_time=start_ns
        )
        if agent_span.error:
            otel_span.set_status(trace.StatusCode.ERROR, str(agent_span.error))
            otel_span.record_exception(Exception(str(agent_span.error)))
